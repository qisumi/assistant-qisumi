...
interface StepEditableFieldProps {
    ...
    taskId: number;
    onUpdate?: () => void;
    compact?: boolean; // If true, only show status and estimate (for right-side layout)
}

export const StepEditableField: React.FC<StepEditableFieldProps> = ({ step, taskId, onUpdate, compact = false }) => {
    ...
    return (
        <div>
            {!compact ? (
                        ...
                        {editingEstimate ? (
                            ...
                            >
                                <Space size={4}>
                                    ...
                                    <Text style={{ fontSize: 'clamp(0.625rem, 1vw, 0.75rem)', color: step.estimateMinutes ? '#8c8c8c' : '#d9d9d9' }}>
                                        {formatEstimate(step.estimateMinutes ?? null).label}
                                    </Text>
                                </Space>
                            </div>
                        )}
                ...
                /* Compact mode: only status and estimate */
                <Space size="small" wrap>
                    {editingStatus ? (
                        <Select
                            value={statusValue}
